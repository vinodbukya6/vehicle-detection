...
model_path = "best2_integer_quant.tflite"
#loading a YOLO model
# model = YoloDetectorWrapper('best2_integer_quant.tflite') #yolov8n.pt, best.pt
model = YOLO(model_path, task='detect')

# Set RTSP streaming address
rtsp_stream_address = "rtsp://127.0.0.1:8554/live.stream" # rtsp://192.168.1.145:8554/live.stream
...
def vehicledetctionsystem(rtsp_video_feed):
    ...
    if input_file:
        ...
        frames_list = []
        cy_linha = 320 #int(1700 * scale_percent/100 ) # 1500
        cx_sentido = int(2000 * scale_percent/100)
        offset = int(30 * scale_percent/100 ) #8
        contador_in = 0
        contador_out = 0
        ...
        tmp_output_path = "./temp_outputs/" + "video_roi0.avi"

        #fourcc=cv2.VideoWriter_fourcc(*'XVID')
        #output_video=cv2.VideoWriter(tmp_output_path,fourcc,fps,(width,height))

        ### RTSP output ####
        # Start ffmpeg process to receive video from the camera and stream it via RTSP
        ffmpeg_process = subprocess.Popen(['ffmpeg', '-f', 'lavfi', '-i', 'anullsrc',
                                           '-f', 'rawvideo', '-pixel_format', 'bgr24',
                                           '-video_size', f'{img_width}x{img_height}',
                                           '-framerate', f'{fps}',
                                           '-i', '-', '-c:v', 'libx264', '-preset',
                                           'ultrafast', '-tune', 'zerolatency',
                                           '-f', 'rtsp', '-rtsp_transport', 'tcp', rtsp_stream_address], stdin=subprocess.PIPE)
        ...
        while True:
            ...
            if True:
                ...
                cv2.line(frame, (0, cy_linha), (int(4500 * scale_percent/100 ), cy_linha), (255,255,0), 2)
                ...
                for ix, row in enumerate(positions_frame.iterrows()):
                    ...
                    #print(center_y, cy_linha, offset, center_x, cx_sentido)
                    ...
                    if (center_y < (cy_linha + offset)) and (center_y > (cy_linha - offset)):
                        # for two lanes ===> <--lane1(in)-->cx_sentido <--lane2(out)-->
                        ...
                        # for one lane ===> combine all in and out once
                        contador_out += 1
                        veiculos_contador_out[category] += 1
                        #print(center_y, cy_linha, offset, center_x, cx_sentido)
                ...
                #print(contador_in_plt, contador_out_plt)
                ...
                xt = 40
                for txt in range(len(contador_out_plt)):
                    # print(txt, contador_out_plt)
                    # write on frame only detected classes
                    if "0" not in contador_out_plt[txt]:
                        xt +=30
                        # number of vehicles heading
                        cv2.putText(img=frame, text='N. vehicles Out',
                                    org= (int(2800 * scale_percent/100 ),30),
                                    fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1, color=(255, 255, 0),thickness=1)
                ...
                # Write the raw frame to the stdin of the ffmpeg process
            ...
            count += 1
            #  save as dataframe: Date and Detections
            now = datetime.now()
            dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
            #print(dt_string, contador_out_plt)
            df = pd.DataFrame({'Date_and_Time': [dt_string], 'Detections': [contador_out_plt]})
            temp_list.append(df)
            final_df = pd.concat(temp_list)
            final_df.to_csv('predicted_classes.csv', index=False)
