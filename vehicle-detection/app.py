...
app = Flask(__name__)
INPUT_FOLDER = os.path.join('static', 'uploads')
...
@app.route('/vehicle-detection-system', methods=['POST'])
def vehicledetctionsystem():
    # input request file
    input_file = request.files["uploadfile"]
    
    if input_file:

        filename = secure_filename(input_file.filename) # Secure the filename to prevent some kinds of attack
        #file path where we have to save 
        file_path = os.path.join(app.config['INPUT_FOLDER'], filename)
        # save file, remove after processing completes
        ...
        return render_template('result.html', user_image=temp)
