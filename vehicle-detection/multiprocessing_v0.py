...
def feed_read(stack) -> None:
    ...
    frames_list = []
    cy_linha = 500 #750 #int(1500 * scale_percent/100 ) # modify
    cx_sentido = int(2000 * scale_percent/100) # modify
    offset = 100 #int(30 * scale_percent/100 ) # modify
    contador_in = 0
    contador_out = 0
    ...
    while True:
        if len(stack) != 0:
            ...
            existing_row = df[df['Date_and_Time'] == dt_string] # total_objects_crossed
            # Add the new row if it does not exist
            if contador_out and existing_row.empty:
...
if __name__ == '__main__':
    q = Manager().list()
    rtsp_url = "rtsp://admin:rr123456@192.168.1.88:554/Streaming/Channels/1"  # Replace with your camera URL
    pw = Process(target=feed_write, args=(q, rtsp_url, 100))
